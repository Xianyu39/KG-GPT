    ...
    for divided_claim, corresponding_entity_set in divided_claim_list.items():
        ...
        if len(corresponding_entity_set) == 1:
            if len(list(KG[corresponding_entity_set[0]])) == 1:
                ...
                relation_retrieval_query = open_file('./meta_1hop_prompts/relation_retrieval_prompt.txt').replace('<<<<SENTENCE>>>>', divided_claim).replace('<<<<RELATION_SET>>>>', str(one_hop_relation_set))
        else:
            relation_retrieval_query = open_file('./meta_1hop_prompts/relation_retrieval_prompt.txt').replace('<<<<SENTENCE>>>>', divided_claim).replace('<<<<RELATION_SET>>>>', str(new_all_relation_set))
